    ...
    match command {
        ...
        SandboxCommands::Remove {
            ...
            ExecutionMode::Direct => {
                Err(CliError::not_implemented(format!("osbase sandbox remove {target}")))
            }
        },
        SandboxCommands::List { json } => handle_sandbox_list(mode, json),
        SandboxCommands::Status { target, .. } => match mode {
            ...
            ExecutionMode::Direct => {
                Err(CliError::not_implemented("osbase sandbox status"))
            }
        },
    }
}
    ...
    match mode {
        ...
        ExecutionMode::Direct => {
            ...
            match osbase_install::execute_install(&request, &env) {
                ...
                Err(err) => Err(map_osbase_err(err, &target)),
            }
        }
    }
}

fn handle_sandbox_list(mode: ExecutionMode, json: bool) -> Result<(), CliError> {
    match mode {
        ExecutionMode::ViaHelper(mut stream) => {
            let req = HelperRequest::OsbaseList { filter: None };
...
fn map_osbase_err(err: OsbaseInstallError, target: &str) -> CliError {
