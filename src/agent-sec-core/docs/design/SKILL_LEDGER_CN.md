...
| 内部 resolver | 写入运行态 activation（供测试和后续 daemon 复用，不提供 CLI） | 已实现 |
...
本阶段先实现 resolver 核心能力，供测试和后续 daemon 模式复用。Skill Ledger 不提供面向用户或 SkillFS 的 `resolve` CLI；activation refresh 是 daemon 内部职责。resolver 根据当前版本链和默认策略 `pass_only` 选择可运行 snapshot，并原子写入 `.skill-meta/activation.json`，同时尽力同步写入 skill 目录 xattr `user.agent_sec.skill_ledger.activation`：
...
resolver 始终只激活 snapshot，不激活 source/current 工作区。当前工作区处于 `drifted`、`warn`、`deny` 或 `tampered` 时，会继续指向最近可信 pass snapshot；若不存在可信版本，则 `target=null`。后续 daemon 模式会负责在文件变化、扫描完成或重启 reconcile 时调用该 resolver。
